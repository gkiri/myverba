...
class GeminiGenerator(Generator):
    ...
    def prepare_messages(
        self, queries: list[str], context: list[str], conversation: dict[str, str], pdf_data: bytes = None
    ):
        """
        ...
        @parameter pdf_data: A bytes object representing the PDF data to be included in the messages.
        ...
        if pdf_data:
            pdf_data_base64 = base64.standard_b64encode(pdf_data).decode("utf-8")
            parts.append(Part.from_inline_data(
    ...
    async def generate_pdf_nostream(
        ...
        """Generate a response for PDF analysis using Gemini"""
        ...
        try:
            # Initialize Vertex AI (existing code)
            project_id = os.getenv("GOOGLE_CLOUD_PROJECT")
            REGION = "us-central1"
            credentials_path = os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
            if credentials_path:  # Check if credentials path is set
            ...
            # Prepare messages using the shared method
            ...
            # Generate content using prepared messages
            generative_multimodal_model = GenerativeModel(
                model_name if model_name else self.model_name
            )
            completion = await generative_multimodal_model.generate_content_async(
                contents=messages  # Use prepared messages instead of manual content
            )

            yield {
                "message": completion.candidates[0].content.parts[0].text,
                "finish_reason": completion.candidates[0].finish_reason,
            }

        except Exception as e:
            msg.fail(f"Generation error: {e}")
            raise